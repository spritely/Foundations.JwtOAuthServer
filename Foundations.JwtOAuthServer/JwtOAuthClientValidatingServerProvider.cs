// --------------------------------------------------------------------------------------------------------------------
// <copyright file="JwtOAuthClientValidatingServerProvider.cs">
...
namespace Spritely.Foundations.JwtOAuthServer
{
    ...
    public class JwtOAuthClientValidatingServerProvider : OAuthAuthorizationServerProvider
    {
        ...
        public override Task ValidateClientAuthentication(OAuthValidateClientAuthenticationContext context)
        {
            ...
            if (!ServerSettings.AllowedClients.Any(c => c.Id == context.ClientId))
            {
                context.SetError(
                    "invalid_client_id",
        ...
        [SuppressMessage("Microsoft.Design", "CA1031:DoNotCatchGeneralExceptionTypes",
            Justification =
                "We do not know what exception types a user may throw so we scope the handling as tight as possible and catch everything."
            )]
        public override Task GrantResourceOwnerCredentials(OAuthGrantResourceOwnerCredentialsContext context)
        {
            ...
            try
            {
                identity = Authenticator.SignIn(credentials);
            }
            catch (Exception ex)
            {
                Log.Write(ex);
